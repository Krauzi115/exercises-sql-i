...
SELECT genres.id, genres.name, COUNT(*) AS track_count_num
FROM genres
JOIN tracks
...
SELECT genres.id, genres.name, COUNT(*) AS track_milli
FROM genres
JOIN tracks
  ON (tracks.milliseconds = genres.id)
GROUP BY genres.id
ORDER BY track_milli DESC
LIMIT 5;
---this one gives me trouble

---coming back to this one
-- The top 5 genres by average track length (in milliseconds)
SELECT genres.id, genres.name, AVG(milliseconds) AS avg_track_milli
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
ORDER BY avg_track_milli DESC
LIMIT 5;
...
SELECT albums.id, albums.title, SUM(milliseconds) AS play_time_long
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
ORDER BY play_time_long DESC
LIMIT 10;
-- The 10 highest-selling tracks of all time
-- Hint: you'll need to join the tracks table and the invoice_lines table
SELECT tracks.id, tracks.name, SUM(unit_price) AS highest_selling_tracks
FROM tracks
JOIN invoice_lines
  ON (invoice_id = tracks.id)
GROUP BY tracks.id
ORDER BY highest_selling_tracks DESC
LIMIT 10;
