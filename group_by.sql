...
SELECT billing_city, AVG(total) AS avgerage_invoice_size
FROM invoices
WHERE billing_country = 'USA'
GROUP BY billing_city
ORDER BY avg_invoice_size DESC
...
SELECT billing_city, AVG(total) AS average_invoice_size
FROM invoices
WHERE billing_country = 'USA' AND billing_state = 'CA'
GROUP BY billing_city
ORDER BY avg_invoice_size_num DESC
LIMIT 10;
...
SELECT country, COUNT(*) AS customer_total_num
FROM customers
GROUP BY country
ORDER BY customer_count DESC
LIMIT 3;
-- A list of the top 7 cities (anywhere) by total number of customers
SELECT city, COUNT(*) AS customer_total_num
FROM customers
GROUP BY city
ORDER BY customer_count DESC
LIMIT 7;
